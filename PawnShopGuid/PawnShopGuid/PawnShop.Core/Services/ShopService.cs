...
namespace PawnShop.Core.Services
{
    public class ShopService : IShopService
    {
        ...
        public async Task BuyAsync(Guid id)
        {
        ...
        public async Task<AllGoodsInShopViewModel?> DeleteAsync(Guid? id)
        {
            var goods = await repository.AllReadOnly<Shop>()
             .Where(a => a.Id == id)
        ...
        public async Task EditAsync(Guid id, EditGoodsInShop model)
        {
            var goods = await repository.All<Shop>()
                 .Where(x => x.Id == id)
                 .Where(x => x.IsDeleted == false)
        ...
        public async Task<EditGoodsInShop?> FindAsync(Guid? id)
        {
            var goods = await repository.All<Shop>()
                .Where(x => x.Id == id)
                .Where(x => x.IsDeleted == false)
